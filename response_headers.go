...
// to [ResponseHeader.Set] unless the header explicitly needs to be overwritten
// Header key and values will be sanitised per HTTP spec before being added to
// the server's response. It is the user's responsibility to ensure that the
// headers are safe and non-conflicting. For example, it is heavily discouraged
// to modify the Content-Type or Content-Length headers as they are managed
// implicitly whenever a body is written to a response and can cause issues on
// the client if they contain incorrect values.
